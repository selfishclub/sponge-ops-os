...
| 스폰지클럽 저장소 | `1_mission/` 과 `3_session_skills/` 가 같이 있음 | **여기에 만들지 않는다** | 닉네임을 묻고 제출물을 읽은 뒤, 내 프로젝트 폴더 경로를 묻는다. 프로젝트가 없으면 "코드 없음"으로 |
| 코드 없음 | 둘 다 아님 | 사용자가 정한 폴더의 `docs/ops-os/` (기본: 지금 폴더) | 랜딩과 무관한 운영일 가능성이 크다 → 사다리 1·2칸 우선 |
...
표와 마지막 줄 `추천:` 을 출력한다. 아무것도 쓰지 않는다.
...
| 이어서 구현 | 모드 ① 7단계부터 (또는 vN 이면 모드 ② 4단계) |
| ② 다시 / ② 이어서 | ② |
| 아직 이르다 | log 한 줄 적는 법만 안내하고 끝. 사용자가 원하면 ② 로 가되 숫자에 "추정" |
...
"손으로", "매번", "직접 보냅니다", "옮겨 적" 같은 표현은 흐름 속 손일 후보다. 그대로 인용해 근거로 적는다.

## 4. 저장소 운영 흔적

```bash
# 어드민 · 저장 표 · 예약
ls -d app/admin src/app/admin pages/admin app/tools src/app/tools 2>/dev/null
...
# 발송 코드 (메일·문자·알림톡)
grep -rliE "resend|nodemailer|sendgrid|solapi|aligo|coolsms|twilio|alimtalk" --include='*.ts' --include='*.tsx' --include='*.js' --include=package.json --exclude-dir=node_modules . 2>/dev/null | head
...
grep -rlE "docs.google.com/spreadsheets|script.google.com" --include='*.md' --include='*.ts' --include='*.tsx' --exclude-dir=node_modules . 2>/dev/null | head -5
...
브라우저 도구 확인은 `21-browser-agents.md` F 절. Claude in Chrome 은 세션에 `mcp__claude-in-chrome__` 도구가 보이는지로 판단한다.
...
| 시트·Apps Script 흔적 | | §4 |
| 저장소 공개 여부 | public / private / 확인 불가 | gh |
...
운영 인원, 사업 모양, 손일의 주기·시간, 털어놓기, 준비물(사업자등록·카카오 비즈니스 채널·홈택스 인증 방식·구글 계정), 기준선. 저장소나 제출물에서 이미 확인된 것은 묻지 않는다.
