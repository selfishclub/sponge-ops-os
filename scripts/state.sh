...
if [ -d 1_mission ] && [ -d 3_session_skills ]; then ctx="스폰지클럽 저장소"
...
map_rows=0; plans=""; n_run=0; n_build=0; n_plan=0; latest_ship=""; log_rows=0; recent_logs=0; last_log=""
if [ -f "$D/00-map.md" ]; then
  map_rows=$(awk '/^## 3\./{f=1;next} /^## /{f=0} f && /^\| *[0-9]+ *\|/{c++} END{print c+0}' "$D/00-map.md")
fi
for f in "$D"/v*-plan.md; do
  [ -f "$f" ] || continue
  v=$(fm "$f" version); st=$(fm "$f" status); sh=$(fm "$f" shipped_at)
  plans="$plans ${v:-?}:${st:-?}${sh:+($sh)}"
  case "$st" in
    운영중) n_run=$((n_run+1)); if [ -n "$sh" ] && { [ -z "$latest_ship" ] || [[ "$sh" > "$latest_ship" ]]; }; then latest_ship="$sh"; fi ;;
...
if [ "$ctx" = "스폰지클럽 저장소" ]; then
  rec="프로젝트 경로 필요 — 스폰지클럽 저장소에는 docs/ops-os 를 만들지 않는다"
  ...
  elif [ "$recent_logs" -ge 5 ] || { [ -n "$run_days" ] && [ "$run_days" -ge 7 ]; }; then
    rec="② 다시"; [ "$n_run" -ge 2 ] && rec="$rec · v3 연결 후보 (운영중 ${n_run}개)"
  else
    rec="아직 이르다 — log ${recent_logs}줄 · ${run_days:-?}일째. 5줄 또는 7일 뒤에 다시"
  fi
