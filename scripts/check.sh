...
for k in '| 일 |' '| 영역 |' '| 누가 |' '| 주기 |' '한 번(분)' '주당(분)' '실수 위험' '사람만 하는 단계' '준비·승인' '준비 난이도' '자동화 경로' '| 점수 |' '| 출처 |' '## 3. 운영 지도'; do
  grep -qF "$k" templates/00-map.md || say "00-map.md 에 '$k' 없음"
done
for k in '^status:' '^shipped_at:' '^baseline_min_per_week:' '^baseline_source:' '## 8\. 줄어드는 것' 'v2 깊게' 'v2 넓게' 'v3 연결' '멈추는 곳'; do
  grep -qE "$k" templates/v1-plan.md || say "v1-plan.md 에 '$k' 없음"
done
for k in '지난 버전이 줄인 것' '^direction:' '^from:' '^status:' '^shipped_at:'; do
  grep -qE "$k" templates/vN-plan.md || say "vN-plan.md 에 '$k' 없음"
done
for k in '| 날짜 |' '걸린 분' '건너뜀' '주간 합계'; do
...
t=$(mktemp -d)
bash scripts/state.sh "$t" | grep -q '추천: ① 처음' || say "state.sh: 빈 폴더에서 ① 처음을 추천하지 않음"
mkdir -p "$t/club/1_mission" "$t/club/3_session_skills"
